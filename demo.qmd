...
It sits somewhere between between a git history, a database, a browser,
and a process orchestrator.
...
To demo it here, I'm going to show guild without any framework. Just a
simple R script named "experiment.R", and all it does does is a random
walk for 10 steps.
...
The natural thing to do with a script like this is to run it. You can
run it by calling the "source" function, (in the IDE you might click the
"source" button), or at the terminal you might type
...
the run. In R or at the terminal, it's a command called "guild run".
...
The run is the primary unit of composition. After a run, you can view
the run record produced in couple of ways. Guild comes built-in with
interfaces for viewing, managing, and analyzing runs in four contexts:
from the browser, from R, from python, and from the terminal.

To start however, I'm going to show you first
