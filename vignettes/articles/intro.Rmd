...
the previous one. We can view a comparison via the `compare_runs()`
function:
...
Note that `compare_runs()` will by default compare the last two runs,
...
Now, when we inspect the output with `compare_runs()`, we see that the
