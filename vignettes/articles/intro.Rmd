...
`runs_info()` returns a data frame with information about runs. In our
sample project, we've launched one run, so `runs_info()` returns a 1-row
data frame.
...
`runs_info()` as a nested dataframe under `flags`.
...
`runs_info()` by default only returns a summary of run scalars, but the
full scalar history can also be accessed from R directly:

```{r}
runs_info(scalars = TRUE)
```
...
```{r paged.print=FALSE}
runs <- runs_info(paste0("1:", length(units))) # last 8 runs
